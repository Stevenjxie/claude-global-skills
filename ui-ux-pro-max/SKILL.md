...
For UI reviews and focused fixes, read the relevant priority category. This is also the fallback when Python searches are unavailable. [Read the reference](references/ux-checklist.md).
...
### Step 2: Generate Design System (new or explicitly redesigned systems)

Use `--design-system` when the user needs a new design system or explicitly requests replacing one. Preserve existing project tokens for component work, UI reviews, and scoped fixes; follow the scenario table and query only the relevant domain instead.

Resolve every script command below against this skill's actual loaded directory. `.claude/skills/ui-ux-pro-max` is a historical example location, not a required installation path; on Codex use its current skill directory and keep the working directory at the user's project.
...
Read only when an end-to-end search example helps; use the main workflow for routine queries. [Read the reference](references/workflow-example.md).
